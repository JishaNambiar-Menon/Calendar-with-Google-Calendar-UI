...
export default function Timepicker1() {
  ...
  const [eventStart, setEventStart] = useState(dayjs(endDaySelected));
  ...
  function handleChange(newValue, actionMeta) {
    ...
    if (
      ...
      setTempEnd(newValue.value.split(":"));
  ...
  /*function calculateNewDate(e) {
    ...
    if (temp.length !== 0) {
      ...
      setEndDaySelected(
        endDaySelected.set("hour", temp[0]).set("minute", temp[1])
      );
  ...
  return (
    <div className="pt-1">
      <CreatableSelect
        ...
        defaultValue={{
          value: `${String(new Date(endDaySelected).getHours()).padStart(
            2,
            "0"
          )}:${String(new Date(endDaySelected).getMinutes()).padStart(2, "0")}`,
          label: `${String(new Date(endDaySelected).getHours()).padStart(
            2,
            "0"
          )}:${String(new Date(endDaySelected).getMinutes()).padStart(2, "0")}`,
        }}
